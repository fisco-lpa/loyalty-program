...
func InsertAccount(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//往账户表插入数据
	ok, err := stub.InsertRow(chaincode_common.Account, shim.Row{
		Columns: []*shim.Column{
	...
	col := shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account}}
	...
	if len(row.Columns) == 0 {
		...
		ok, err := stub.InsertRow(chaincode_common.Table_Count, shim.Row{
			Columns: []*shim.Column{
				&shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account}}, //表名
				&shim.Column{Value: &shim.Column_Int64{Int64: totalNumber}}},                 //总数
		})
		if !ok && err == nil {
			stub.DeleteRow(chaincode_common.Account, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: data.AccountId}}})
	...
	//把获取的总数插入行号表中当主键
	ok, err = stub.InsertRow(chaincode_common.Account_Rownum, shim.Row{
		Columns: []*shim.Column{
	...
	if !ok && err == nil {
		stub.DeleteRow(chaincode_common.Account, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: data.AccountId}}})
		if len(row.Columns) == 0 {
			stub.DeleteRow(chaincode_common.Table_Count, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account}}})
		} else {
			totalNumber = totalNumber - 1
			ok, err := stub.ReplaceRow(chaincode_common.Table_Count, shim.Row{
				Columns: []*shim.Column{
...
func InsertAccountType(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//往账户表插入数据
	ok, err := stub.InsertRow(chaincode_common.Account_Type, shim.Row{
		Columns: []*shim.Column{
			&shim.Column{Value: &shim.Column_String_{String_: data.AccountTypeId}}, // 账户类型ID
	...
	//判断总数表是否有数据，若有数据则查出来+1
	var columns []shim.Column
	col := shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account_Type}}
	columns = append(columns, col)
	row, _ := stub.GetRow(chaincode_common.Table_Count, columns) //row是否为空
	totalNumber := row.Columns[1].GetInt64()
	if len(row.Columns) == 0 {
		...
		ok, err := stub.InsertRow(chaincode_common.Table_Count, shim.Row{
			Columns: []*shim.Column{
				&shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account_Type}}, //表名
				&shim.Column{Value: &shim.Column_Int64{Int64: totalNumber}}},                      //总数
		})
		if !ok && err == nil {
			stub.DeleteRow(chaincode_common.Account_Type, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: data.AccountTypeId}}})
	...
	//把获取的总数插入行号表中当主键
	ok, err = stub.InsertRow(chaincode_common.Account_Type_Rownum, shim.Row{
		Columns: []*shim.Column{
			&shim.Column{Value: &shim.Column_Int64{Int64: totalNumber}},             //行号
	...
	if !ok && err == nil {
		stub.DeleteRow(chaincode_common.Account_Type, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: data.AccountTypeId}}})
		if len(row.Columns) == 0 {
			stub.DeleteRow(chaincode_common.Table_Count, []shim.Column{shim.Column{Value: &shim.Column_String_{String_: chaincode_common.Account_Type}}})
		} else {
			totalNumber = totalNumber - 1
			ok, err := stub.ReplaceRow(chaincode_common.Table_Count, shim.Row{
				Columns: []*shim.Column{
...
func updateAccount(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	columns = append(columns, col)
	row, _ := stub.GetRow(chaincode_common.Account, columns)
	if len(row.Columns) != 0 {
		ok, err := stub.ReplaceRow(chaincode_common.Account, shim.Row{
			Columns: []*shim.Column{
				&shim.Column{Value: &shim.Column_String_{String_: data.AccountId}},              //账户ID
